              ...
              <div className="flex flex-wrap gap-1 font-medium">
                {item.last.drawDetail.winningNumbers?.map((winning, i) =>
                  winning.numbers.map((num, j) => (
                    <span
                      key={`${i}-${j}`}
                      className="flex items-center justify-center bg-[#ea0029] rounded-full text-white text-[19px] px-2 py-1 h-[35px] w-[35px] mt-2"
                    >
                      {num}
                    </span>
                  ))
            ...
            <div className="relative bg-gradient-to-b from-[#DF081A] to-[#FB636F] w-[228px] text-white ml-auto flex p-2 flex-col justify-between items-end">
              <div className="flex flex-col">
                <div className="text-white text-end text-[12px] mt-3">
                  SIRADAKİ ÇEKİLİŞ{" "}
                  <span className="font-bold">{lastDrawTime}</span>
                </div>
              ...
              <div className="flex bg-white rounded-full text-[#ea0029] text-center justify-center items-center text-sm bottom-0 right-0 w-[137px] h-[32px] ">
                HEMEN OYNA
              </div>
            </div>
          </div>
